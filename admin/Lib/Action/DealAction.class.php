...
class DealAction extends CommonAction{
	...
	public function add()
	{
		...
		//查找服务
		$sql = "select * from ".DB_PREFIX."hotel_fuwu";
		$fuwu_list = $GLOBALS['db']->getAll($sql);
		$this->assign("fuwu_list",$fuwu_list);

		//查找属性
		$sql = "select * from ".DB_PREFIX."hotel_shuxing";
		$shuxing_list = $GLOBALS['db']->getAll($sql);
        $this->assign("shuxing_list",$shuxing_list);

		//查找ktv包厢类型
		/*$sql = "select * from ".DB_PREFIX."ktv_room_type";
		$ktv_room_type_list = $GLOBALS['db']->getAll($sql);
		$this->assign("ktv_room_type_list",$ktv_room_type_list);*/
	...
	public function insert() {
		...
		$data['hotel_fuwu'] = implode(',',$_REQUEST['hotel_fuwu']);
		$data['hotel_shuxing'] = implode(',',$_REQUEST['hotel_shuxing']);
//		$data['ktv_room_type'] = implode(',',$_REQUEST['ktv_room_type']);

//		$this->error($data['hotel_shuxing']);
...
//		$this->error($data['hotel_fuwu']);
		$list=M(MODULE_NAME)->add($data);
		if (false !== $list) {
			$sql = "update ".DB_PREFIX."deal set `hotel_fuwu` = '".$data['hotel_fuwu']."' , `hotel_shuxing` = '".$data['hotel_shuxing']."'  where id = '$list'";
...
//			$this->error($sql);
	...
	public function edit() {		
        ...
        $vo['hotel_fuwu'] = explode(',',$vo['hotel_fuwu']);
        $vo['hotel_shuxing'] = explode(',',$vo['hotel_shuxing']);
//        $vo['ktv_room_type'] = explode(',',$vo['ktv_room_type']);
//		print_r($vo);exit;
		$this->assign ( 'vo', $vo );
		...
		//查找服务
		$sql = "select * from ".DB_PREFIX."hotel_fuwu";
		$all_fuwu = $GLOBALS['db']->getAll($sql);
		foreach($all_fuwu as $key=>$val){

				if(in_array($val['id'],$vo['hotel_fuwu'])){
					$all_fuwu[$key]['is_checked'] = '1';
				}else{
					$all_fuwu[$key]['is_checked'] = '0';
				}

		}

		$this->assign("fuwu_list",$all_fuwu);

		//查找属性
		$sql = "select * from ".DB_PREFIX."hotel_shuxing";
		$all_shuxing = $GLOBALS['db']->getAll($sql);


		foreach($all_shuxing as $key=>$val){

				if(in_array($val['id'],$vo['hotel_shuxing'])){
					$all_shuxing[$key]['is_checked'] = '1';
				}else{
					$all_shuxing[$key]['is_checked'] = '0';
				}
		}

		$this->assign("shuxing_list",$all_shuxing);



		//查找ktv包厢类型
	/*	$sql = "select * from ".DB_PREFIX."ktv_room_type";
		$all_ktv_room_type = $GLOBALS['db']->getAll($sql);


		foreach($all_ktv_room_type as $key=>$val){

			if(in_array($val['id'],$vo['ktv_room_type'])){
				$all_ktv_room_type[$key]['is_checked'] = '1';
			}else{
				$all_ktv_room_type[$key]['is_checked'] = '0';
			}
		}

		$this->assign("ktv_room_type_list",$all_ktv_room_type);*/
...
//		$data['ktv_room_type'] = implode(',',$_REQUEST['ktv_room_type']);
		...
		if (false !== $list) {
            $sql= "update ".DB_PREFIX."deal set `hotel_shuxing` = '".$data['hotel_shuxing']."',`hotel_fuwu` = '".$data['hotel_fuwu']."' where id = '{$data['id']}'";
//			$this->error($sql);
			$ret = $GLOBALS['db']->query($sql);
			$deal_stock = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_stock where deal_id = '".$data['id']."'");
			if(!$deal_stock)
