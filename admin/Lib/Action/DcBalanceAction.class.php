...
class DcBalanceAction extends CommonAction{
	...
	public function bill()
	{
		...
		if(strim($_REQUEST['name'])!='')
		{
			...
			$list = $GLOBALS['db']->getAll("select s.* from ".DB_PREFIX."supplier as s left join ".DB_PREFIX."supplier_location as sl on s.id=sl.supplier_id where s.name like '%".strim($_REQUEST['name'])."%' and (sl.is_dc=1 or sl.is_reserve=1) group by s.id limit ".$limit);
		    $total = $GLOBALS['db']->getOne("select count(*) from (select s.* from ".DB_PREFIX."supplier as s left join ".DB_PREFIX."supplier_location as sl on s.id=sl.supplier_id where s.name like '%".strim($_REQUEST['name'])."%' and (sl.is_dc=1 or sl.is_reserve=1) group by s.id) t1");

		}
		else
		{

			$list= $GLOBALS['db']->getAll("select s.* from ".DB_PREFIX."supplier as s left join ".DB_PREFIX."supplier_location as sl on s.id=sl.supplier_id where sl.is_dc=1 or sl.is_reserve=1 group by s.id limit ".$limit);
			$total = $GLOBALS['db']->getOne("select count(*) from (select s.* from ".DB_PREFIX."supplier as s left join ".DB_PREFIX."supplier_location as sl on s.id=sl.supplier_id where sl.is_dc=1 or sl.is_reserve=1 group by s.id) t1");
