...
//         echo $sql . $conditions . " order by id desc limit " . $limit;exit;
    ...
    public function publish()
    {
        ...
        $GLOBALS['tmpl']->assign("fuwu_list", $fuwu_list);
        //查找属性
        $sql = "select * from ".DB_PREFIX."hotel_shuxing";
        $shuxing_list = $GLOBALS['db']->getAll($sql);
        $GLOBALS['tmpl']->assign("shuxing_list", $shuxing_list);
...
//             $deal_info = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "deal where id=" . $id); //仅限开放测试使用
    ...
    public function do_save_publish()
    {
        ...
        $data['city_id'] = intval($_REQUEST['city_id']); // 城市
        $data['hotel_fuwu'] = implode(',',$_REQUEST['hotel_fuwu']); // 服务
        $data['hotel_shuxing'] = implode(',',($_REQUEST['hotel_shuxing'])); // 属性
