...
function get_hiz_gopreview()
{
	$gopreview = es_session::get("hiz_gopreview");
	if($gopreview==get_current_url())
	{
		$gopreview = url("hiz");
	}
...
function set_hiz_gopreview()
{
	//     $url =  get_current_url();
	//     es_session::set("biz_gopreview",$url);
	es_session::set("hiz_gopreview",url("hiz"));
}
...
//显示错误
function showHizErr($msg,$ajax=0,$jump='',$stay=0)
{
	if($ajax==1)
	{
		$result['status'] = 0;
...
//显示成功
function showHizSuccess($msg,$ajax=0,$jump='',$stay=0)
{
	if($ajax==1)
	{
		$result['status'] = 1;
