...
class dcmenuModule  extends MainBaseModule 
{
    ...
    public function add_menu_cart(){
		...
		*/
		...
		if(!$cart_item)
		{	$cart_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_cart where session_id='".$session_id."' and user_id=".$user_id." and menu_id=".$menu_id);
		 	
		 	if($cart_info){
		 		
		 	$cart_item['num']=$cart_info['num']+$number;	
		 	...
		 	$GLOBALS['db']->autoExecute(DB_PREFIX."dc_cart",$cart_item,"UPDATE","id=".$cart_info['id']);	
		 	}else{
