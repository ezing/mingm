...
class goodsoModule extends HizBaseModule
{
	...
	{			
	    ...
	    $sql = "select distinct(doi.id),doi.*, s.name as supplier_name ,s.allow_refund,do.order_status,do.delivery_id,do.memo,do.create_time,do.order_sn,do.total_price,do.pay_amount,doi.refund_status,do.region_lv1,do.region_lv2,do.region_lv3,do.region_lv4,do.consignee,do.address,do.zip,do.mobile from ".$order_item_table_name." as doi left join ".
	...
	public function check_delivery()
	{
		...
		$s_account_info = $GLOBALS["hiz_account_info"];
		$supplier_id = intval($GLOBALS['db']->getRow("select supplier_id from ".DB_PREFIX."deal_order_item where id=".$id));
		$location_list=$GLOBALS['db']->getAll("select id from ".DB_PREFIX."supplier_location where supplier_id=".$supplier_id);
		$s_account_info['location_ids']=array();
		foreach($location_list as $kk=>$vv){
		    $s_account_info['location_ids'][]=$vv['id'];
