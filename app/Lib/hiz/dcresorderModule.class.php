...
class dcresorderModule extends HizBaseModule
{
...
public function index()
	{	
	    ...
	    $GLOBALS['tmpl']->assign("list",$list); 		
		 $GLOBALS['tmpl']->assign("ajax_url", url("hiz","dcresorder"));
		...
		if(!$order_info)
		{
		 ...
		 if(!in_array($order_info['location_id'], $s_account_info['location_ids']))
		 {
	   	 ...
	   	 if($rs_time < NOW_TIME + 1800){  //商家接单时间，要在用户预订时间前半小时，否则关闭订单
	   	 	...
	   	 	$root['jump'] = url("hiz","dcresorder#index");
	   	 	ajax_return($root);
	...
	{	
		...
		if(!$order_info)
		{
		 ...
		 if(!in_array($order_info['location_id'], $s_account_info['location_ids']))
		 {
	   	 ...
	   	 $result['jump'] = url("hiz","dcresorder#index");
		 ajax_return( $result);
	...
	{
		$id = intval($_REQUEST['id']);
		$form_url=url("hiz","dcresorder#close_order",array('id'=>$id));
