...
class locationModule extends HizBaseModule
{
	...
	{		
        ...
        $supplier_list=$GLOBALS['db']->getAll(" select id from " . DB_PREFIX . "supplier".$conditions);
		$s_id=array();
        ...
        $conditions_s=' where supplier_id in ('.$s_str.')';
        ...
        $total = $GLOBALS['db']->getOne($sql_count.$conditions_s);
        $page = new Page($total, $page_size); // 初始化分页对象
        $p = $page->show();
        $GLOBALS['tmpl']->assign('pages', $p);

        $list = $GLOBALS['db']->getAll("select sl.* , s.name as supplier_name from " . DB_PREFIX . "supplier_location as sl left join ".DB_PREFIX."supplier as s on sl.supplier_id=s.id where sl.is_effect=1 and sl.supplier_id in ( ".$s_str.") order by sl.id desc limit " . $limit);
        ...
        foreach ($list as $k => $v) {
        	$list[$k]['edit_url'] = url("hiz", "location#edit", array(
        			"id" => $v['id'],
        			"edit_type" =>1
        	));
        	$list[$k]['preview_url'] = url("index","preview#store",array("id"=>$v['id'],"type"=>0));
        	$list[$k]['update_url'] = url("hiz","location#update",array("id"=>$v['id']));
	    ...
	    $GLOBALS['tmpl']->assign("publish_btn_url", url("hiz", "location#publish"));
	    $GLOBALS['tmpl']->assign("form_url", url("hiz", "location#no_online_index"));
	    $GLOBALS['tmpl']->assign("ajax_url", url("hiz", "location"));
	    $GLOBALS['tmpl']->assign("index_url", url("hiz", "location#index"));
	    $GLOBALS['tmpl']->assign("no_online_index_url", url("hiz", "location#no_online_index"));
	...
	public function no_online_index()
	{
	    ...
	    /* 业务逻辑部分 */
	    $conditions_f .= " where agency_id = ".$account_id; // 查询条件
	    
	    $supplier_list=$GLOBALS['db']->getAll(" select id from " . DB_PREFIX . "supplier".$conditions_f);
	    $s_id=array();
	    foreach($supplier_list as $k=>$v){
	    	$s_id[]=$v['id'];
	    }
	    ...
	    $conditions .= " and supplier_id in ( " . $s_str .")";
	...
	public function do_save_publish(){
	    ...
	    if($id > 0){
	    	$supplier_id = intval($GLOBALS['db']->getOne("select supplier_id from " . DB_PREFIX . "supplier_location  where id=".$id));
	    }else{
	    	$supplier_id= intval($_REQUEST['supplier_id']);
	    }
	   
	    $account_id = intval($GLOBALS['db']->getOne("select id from " . DB_PREFIX . "supplier_account  where supplier_id=".$supplier_id));
	    ...
	    foreach($location_list as $k=>$v){
	    	$account_info['location_ids'][]=$v['id'];
	    }
	    ...
	    if ($id > 0) {
	        ...
	        $result['jump'] = url("hiz","location#no_online_index");
	    } else {
	   ...
	   $location_list= $GLOBALS['db']->getAll("select id from ".DB_PREFIX."supplier_location where supplier_id=".$supplier_id);
	    ...
	    if($edit_type == 1 && $id>0){ //判断是否有存在修改
	        ...
	        if($location_submit_info && $location_submit_info['admin_check_status']==0){
	            showHizErr("已经存在申请操作，请先删除避免重复申请",0,url("hiz","location#index"));
	            exit;
	        }
	    }
	    ...
	    if ($edit_type == 1) {//管理员发布
	        ...
	        if (empty($location_info)) {
	            showHizErr("数据不存在或没有操作权限！",0,url("hiz","location#index"));
	            exit();
	        }
	        ...
	        $go_list_url = url("hiz","location#index");
	        $is_online = 1;
	        ...
	        if (empty($location_info)) {
	            showHizErr("数据不存在或没有操作权限！",0,url("hiz","location#no_online_index"));
	            exit();
	        }
	        
	        $location_images = unserialize($location_info['cache_supplier_location_images']);
	        $go_list_url = url("hiz","location#no_online_index");
	...
	public function do_update(){
        ...
        $result['jump'] = url("hiz","location#index");
	...
	public function del()
	{
	    ...
	    /* 业务逻辑 */
	    if ($GLOBALS['db']->getOne("select count(*) from " . DB_PREFIX . "supplier_location_biz_submit where id=" . $id) ){
	        // 存在切用户有权限删除
	        $GLOBALS['db']->query("delete from " . DB_PREFIX . "supplier_location_biz_submit where id=" . $id );
	        $data['status'] = 1;
	        $data['info'] = "删除成功";
	    } else {
	        $data['status'] = 0;
