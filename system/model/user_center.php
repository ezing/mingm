...
function get_user_log_new($limit,$user_id,$t='')
{
	if(!in_array($t,array("money","score","point")))
	{
		$t = "";
	...
	$user_id = intval($user_id);
	$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_log where user_id = ".$user_id." and deal_id >0 $condition order by id desc limit ".$limit);



	foreach ($list as $k=>$v){
	    $list[$k]['flog_time'] = to_date($v['log_time']);
	    $list[$k]['score_new'] = substr($v['score'],1);

	    $deal_info =  $GLOBALS['db']->getRow("select sub_name,img from ".DB_PREFIX."deal where id = ".$v['deal_id']);
	    $list[$k]['sub_name'] = $deal_info['sub_name'];
	    $list[$k]['img'] = $deal_info['img'];
	}
	$count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_log where user_id = ".$user_id." and deal_id >0 $condition");
	return array("list"=>$list,'count'=>$count);
}
