...
class Dc_Wwxjspay_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		{		
			...
			return "<img id='wxscan' src='".url("index","dc_file#dc_wxpay_qr_code",array("notice_id"=>$payment_notice_id))."' /> <br /> 打开微信扫码，即可支付".$script;
		}
		else
		{	 		
	
			$pay['pay_info'] = $title_name;
			$pay['pay_action'] = SITE_DOMAIN.APP_ROOT."/cgi/dc_payment/wwxjspay/redirect.php?notice_id=".$payment_notice_id."&from=".$GLOBALS['request']['from'];
	...
	public function get_redirect_url($payment_notice_id)
	{
		...
		$pay_action = SITE_DOMAIN.APP_ROOT."/cgi/dc_payment/wwxjspay/redirect.php?notice_id=".$payment_notice_id."&from=".$from;
...
// 		var_dump(htmlspecialchars($html_text));exit;
		...
		$html_text = str_replace("__qr_code__", url("index","dc_file#dc_wxpay_qr_code_transapp",array("notice_id"=>$payment_notice['id'],"prepay_id"=>$prepay_id)), $html_text);
