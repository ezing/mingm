...
require_once APP_ROOT_PATH."system/dc_payment/Walipay/alipay_submit.class.php";
require_once APP_ROOT_PATH."system/dc_payment/Walipay/alipay_notify.class.php";
class Dc_Walipay_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$pay['pay_action'] = SITE_DOMAIN.APP_ROOT."/cgi/dc_payment/walipay/redirect.php?notice_id=".$payment_notice_id."&from=".$GLOBALS['request']['from'];
	...
	public function get_redirect_url($payment_notice_id)
	{
		...
		if($from=="wap")
		  	$data_return_url = SITE_DOMAIN.APP_ROOT.'/callback/dc_payment/walipay_response.php';
		else
			$data_return_url = SITE_DOMAIN.APP_ROOT.'/callback/dc_payment/walipay_response_app.php';
		
		$data_notify_url = SITE_DOMAIN.APP_ROOT.'/callback/dc_payment/walipay_notify.php';
...
// 		print_r($alipay_config);exit;
		...
		$html_text = '<script type="text/javascript" src="'.SITE_DOMAIN.APP_ROOT.'/cgi/dc_payment/walipay/ap.js"></script>';
			...
			if ($from == 'wap'){	
						...
						if($rs ==1 || $rs ==2 ){				
								if($order_info['is_cancel']==0){
									if($rs_row)
									{								
										app_redirect(wap_url("index","dc_payment#done",array("order_id"=>$payment_notice['order_id'],'pay_status'=>1))); //支付成功
									...
									else 
									{
										...
										{	//付款单号重复支付,当前支付的退到会员帐户
											app_redirect(wap_url("index","dc_payment#done",array("order_id"=>$payment_notice['order_id'],'pay_status'=>2,'payment_notice_id'=>$payment_notice['id'])));										
											
										}
										else{
											//还有余额未支付，显示继续支付
											app_redirect(wap_url("index","dc_payment#done",array("order_id"=>$payment_notice['order_id'],'pay_status'=>0,'payment_notice_id'=>$payment_notice['id'])));			
										}	
									}
								}else{
										//订单已经关闭
										app_redirect(wap_url("index","dc_payment#done",array("order_id"=>$payment_notice['order_id'],'pay_status'=>3)));
								}
						}
						elseif($rs ==0)
						{
				...
				echo file_get_contents(APP_ROOT_PATH."system/dc_payment/close_page/close_page.html");
			}
