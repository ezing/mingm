...
// 				}
			...
			if(!$cart_item)
			{
				...
				//修改
				//$pintuan_info = $GLOBALS['db']->getRow("select id from ".DB_PREFIX."pin_tuan where deal_id = ".$id);

				if(intval($GLOBALS['request']['price'])){
					$cart_item['unit_price'] = intval($GLOBALS['request']['price']);
					$cart_item['group_id'] = intval($GLOBALS['request']['group_id']);
					$cart_item['total_price'] = $cart_item['unit_price'];
					$cart_item['return_money'] = $deal_info['return_money'];
					$cart_item['return_total_money'] = $deal_info['return_money'];
	...
	{	
		...
		foreach($cart_list_group as $k=>$v)
		{
			...
			$cart_youhui = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."youhui where supplier_id = ".$v['supplier_id']." and begin_time < ".NOW_TIME." and end_time > ".NOW_TIME);
			$cart_youhui_log = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."youhui_log where user_id = ".$GLOBALS['user_info']['id']);



			foreach ($cart_youhui as $k1 => $v1) {
				foreach ($cart_youhui_log as $k2 => $v2) {
					if($v1['id']==$v2['youhui_id']){
		...
		// $root['payment_list1'] = $payment_list;
		foreach($payment_list as $k=>$v)
		{
		...
		$pai_xu = array();
		$add_time = array();
		$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_consignee where  user_id = ".$GLOBALS['user_info']['id']." and is_default = 1");
		foreach($list as $k=>$v){
		...
		array_multisort($pai_xu,SORT_DESC,$add_time,SORT_DESC,$list);
	...
	public function done()
	{
		...
		foreach($goods_list as $k=>$v)
		{
			...
			if(!$data['status'])
				return output(array(),0,$data['info']);
		...
		if($data['is_delivery'] == 1)
		{
			...
			foreach($data['delivery_fee_supplier'] as $k=>$v)
			{
				if($v<0)
				{
					return output(array(),0,"部份商品不支持".$data['delivery_info']['name']);
				}
			}
			
			if(!$consignee_info)
			{
				return output(array(),0,"请设置收货方式");
			}
		}
		...
		foreach($goods_list as $k=>$v)
		{
			...
			$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pin_status = '".$v['group_id']."' where id = ".$order_id);
		...
		// $pin_info = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."pin_tuan where deal_id = ".$deal_ids." and (one_price = ".$order['total_price']." or two_price = ".$order['total_price']." or three_price = ".$order['total_price'].")");
		// $pin_info = $GLOBALS['db']->getOne("select * from ".DB_PREFIX."pin_tuan where deal_id = ".$deal_ids." and id=".$goods_list[0]['group_id']);
		// if($pin_info){
		// 	$g_item = $pin_info;
		// }
		...
		// $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pin_status = '".$g_item."' where id = ".$order_id);
