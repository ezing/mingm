...
class dc_dcorderApiModule extends MainBaseApiModule
{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			if($order_info)
			{
				...
				if($dc_reminder){
					...
					return output($root,0,"亲,您催的太快了,15分钟催一次哦!");
					...
					if($rs > 0){
						...
						return output($root,1,"催单成功,请耐心等待!");
					}else{

						return output($root,0,"催单失败,请重新催单!");
					}
				}
