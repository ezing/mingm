...
class uc_collectApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		if($user_login_status!=LOGIN_STATUS_LOGINED){			
			...
			$result = get_collect_list($limit,$user_id,'in (1,2,3)');
	...
	public function event_collect()
	{
		...
		if(check_login()!=1){			
			...
			$page_size = PAGE_SIZE;
			$limit = (($page-1)*$page_size).",".$page_size;			
			$result = get_event_collect($limit,$user_id,4);
			$page_total = ceil($result['count']/$page_size);
			$list=array();
			foreach($result['list'] as $k=>$v)
			{	
	...
	public function store_collect()
	{
		...
		if(check_login()!=1){			
			...
			$page_size = PAGE_SIZE;
			$limit = (($page-1)*$page_size).",".$page_size;			
			$result = get_store_collect($limit,$user_id,5);
			$page_total = ceil($result['count']/$page_size);
			$list=array();
			foreach($result['list'] as $k=>$v)
			{	
				$list[$k]['id']=$v['id'];
				$list[$k]['cid']=$v['cid'];
				$list[$k]['icon'] = get_abs_img_root(get_spec_image($v['preview'],140,85,1));
				$list[$k]['name']=$v['name'];
			...
			$root['store_list'] = $list?$list:array();			
			
			$root['page'] = array("page"=>$page,"page_total"=>$page_total,"page_size"=>$page_size,"data_total"=>$result['count']);
			
			$root['page_title'] = $GLOBALS['m_config']['program_title']?$GLOBALS['m_config']['program_title']." - ":"";
			$root['page_title'].="店铺收藏";
