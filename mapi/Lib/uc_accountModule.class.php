...
class uc_accountApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		$info = $GLOBALS['db']->getRow("select byear,bmonth,bday,sex,avatar form ".DB_PREFIX."user where id = ".$data['id']);

		//$data
		//get_abs_img_root(get_muser_avatar($user_data['id'],"big"))?get_abs_img_root(get_muser_avatar($user_data['id'],"big")):"";
	...
	public function save()
	{
		...
		$sex = strim($GLOBALS['request']['sex']);
		$brithday = strim($GLOBALS['request']['brithday']);
	...
	public function save_new()
	{
		...
		//$res = save_user($user_data,'UPDATE');
		// $sql = "UPDATE ".DB_PREFIX."user SET user_name = ".$user_name.", email = ".$email.",sex = ".$sex.",brithday = ".$brithday." where id = ".$GLOBALS['user_info']['id'];
		// $res = $GLOBALS['db']->query($sql);
		$res = $GLOBALS['db']->query("update ".DB_PREFIX."user set user_name = '".$user_name."',email='".$email."',sex='".$sex."' where id = ".$user_id );
		// echo "update ".DB_PREFIX."user set user_name = '".$user_name."',email='".$email."',sex='".$sex."' where id = ".$user_id;die;
		if($res)
		{			
			do_login_user($user_data['user_name'],$user_data['user_pwd']);
