	...
	public function wap()
	{
		...
		{		
			...
			foreach($advs as $k=>$v)
			{
				...
				// $adv_list[$k]['img'] = get_abs_img_root(get_spec_image($v['img'], 335, 282,1));  //首页顶部广告图片规格为 宽: 750px 高: 325px
				$adv_list[$k]['img'] = $v['img']; 
		...
		if(!$GLOBALS['m_config']['close_index_supplier'])
		{
			...
			if($indexs_supplier === false)
			{
				...
				foreach($indexs_supplier_rs as $k=>$v){
					...
					$indexs_supplier[$k]['supplier_id'] = $v['supplier_id'];
				}
		...
		$kanjian = $GLOBALS['db']->getAll("select k.id as kid,d.* from ".DB_PREFIX."kan_jia as k left join ".DB_PREFIX."deal as d on k.deal_id=d.id where k.is_recommend=1 limit 0,2");
		$pintuan = $GLOBALS['db']->getAll("select k.id as kid,d.* from ".DB_PREFIX."pin_tuan as k left join ".DB_PREFIX."deal as d on k.deal_id=d.id  where k.is_recommend=1 limit 0,2");
		$event_info = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."event where is_recommend=1 and is_effect = 1 limit 0,2");

		$kanjia_array = array();
		foreach ($kanjian as $key => $value) {
			$a_array = array();
			// $goods_info = $GLOBALS['db']->getRow("select `id`,`sub_name`,`img`,`brief` from ".DB_PREFIX."deal where id = ".$value['deal_id']);
			
			// if($goods_info){
				$a_array['id'] = $value['kid'];
				$a_array['goods_id'] = $value['id'];
				$a_array['sub_name'] = $value['sub_name'];
				$a_array['img'] = $value['img'];
				$a_array['brief'] = $value['brief'];
				$a_array['status'] = 1;						
			// }
			// if($a_array){
			// 	$kanjia_array[] = $a_array;
			// }
			
				$kanjia_array[] = $a_array?$a_array:'';
		}
		$ping_array = array();
		foreach ($pintuan as $k => $v) {
			$b_array = array();
			// $goods_info = $GLOBALS['db']->getRow("select `id`,`sub_name`,`img`,`brief` from ".DB_PREFIX."deal where id = ".$v['deal_id']);
			
			// if($goods_info){
				$b_array['id'] = $v['kid'];
				$b_array['goods_id'] = $v['id'];
				$b_array['sub_name'] = $v['sub_name'];
				$b_array['img'] = $v['img'];
				$b_array['brief'] = $v['brief'];
				$b_array['status'] = 2;	
			// }
			// if($b_array){
			// 	$kanjia_array[] = $a_array;
			...
			$ping_array[] = $b_array?$b_array:'';

		}
		$event_array = array();
		foreach ($event_info as $k2 => $v2) {
			$e_array = array();
			$e_array['id'] = $v2['id'];
			$e_array['sub_name'] = $v2['name'];
			$e_array['img'] = $v2['icon'];
			$e_array['brief'] = $v2['content'];
			$e_array['status'] = 3;
			$event_array[] = $e_array?$e_array:array();;
		}
	
		// $root['huodong_info'] = $kanjia_array;
		$root['huodong_info'] = array_slice(array_merge($kanjia_array,$ping_array,$event_array), 0,5);
		foreach($root['huodong_info'] as $key=>$val){
			$root['huodong_info'][$key]['brief'] =  strip_tags($val['brief']) ;
		}
		//推荐优惠券
