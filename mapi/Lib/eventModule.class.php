...
class eventApiModule extends MainBaseApiModule
{
    ...
    public function index(){
        ...
        foreach($dp_list['list'] as $key=>$val){
            $point_sum += $val['point'];
            $xingxing = '';
            if($val['point'] != 0){
                for($i=0;$i<5;$i++){
            ...
            $dp_list['list'][$key]['xingxing'] = $xingxing;
        }
        //计算平均评分
        $point_count = count($dp_list['list']);
        $pingjun_point = ceil($point_sum/$point_count);
        $pingjun_xingxing = '';
        for($i=0;$i<5;$i++){
            if($i < $pingjun_point){
...
//      print_r($dp_list);exit;
        ...
        //是否收藏
        $con_info = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."deal_collect where id_value = ".$data_id." and user_id = ".$user_id);
        if($con_info){
            $root['is_collect'] = 1;
        }
        return output($root);
    ...
    public function addcollect(){
        $id_value = intval($GLOBALS['request']['id_value']);
        ...
        if($user_login_status!=LOGIN_STATUS_LOGINED){
            ...
            $sql = "INSERT INTO `".DB_PREFIX."deal_collect` (`id`,`id_value`, `user_id`, `create_time`,`type`) select '0','".$id_value."','".$user_id."','".get_gmtime()."','".$type."' from dual where not exists (select * from `".DB_PREFIX."deal_collect` where `id_value`= '".$id_value."' and `user_id` = ".$user_id.")";
