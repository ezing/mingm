    ...
    public function index(){
        ...
        if($user_login_status!=LOGIN_STATUS_LOGINED){
            ...
            if($n_valid){  //已经失效的代金券   use_limit可用次数 0 表示无限  ，use_count已使用次数 ，end_time 有效期必须大于当前时间 
                $condtion = ' and ( et.end_time<'.NOW_TIME.') or ( e.use_limit>0 and e.use_limit = e.use_count)';
            }else{
                $condtion = ' and (e.use_limit=0 or e.use_limit > e.use_count) and et.end_time>'.NOW_TIME;
            }
            ...
            foreach ($list as $k=>$v){
                $temp_arr = array();
                $temp_arr['id'] = $v['id'];
                if(($v['use_limit'] == $v['use_count'] && $v['use_limit']>0) || $v['end_time'] <= NOW_TIME)
