...
class dealApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		$c = intval($GLOBALS['request']['c']);//酒店标识	
...
//		print_r($data);exit;
		...
		$point_count = count($format_dp_list);
		...
		$data['pingjun_xingxing'] = $pingjun_xingxing;
	    sort($data['deal_attr']);
...
//		$data['supplier_id'] = $data['supplier_id'];
	...
	public function detail(){
	    ...
	    if($deal){
	        ...
	        $data['notes']= get_abs_img_root(format_html_content_image($deal['notes'], 150));
	    }

	    $sql = " select * from ".DB_PREFIX."deal_gallery where deal_id= ".$data_id."  order by id desc";
		$promotes = $GLOBALS['db']->getAll($sql);
