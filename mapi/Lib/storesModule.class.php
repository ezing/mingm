...
class storesApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		foreach($list as $k=>$v)
		{
			...
			if($v['deal_cate_match_row']=="KTV"){
				$goodses[$k]['is_ktv'] = 1;
			}else{
				$goodses[$k]['is_ktv'] = 0;
			}
			if($v['deal_cate_match_row']=="酒店住宿"){
				
				$goodses[$k]['is_jiudian'] = 1;
			}else{
				$goodses[$k]['is_jiudian'] = 0;
			}
			if($v['deal_cate_match_row']=="丽人美发"){
				
				$goodses[$k]['is_liren'] = 1;
			}else{
				$goodses[$k]['is_liren'] = 0;
			}			
			//评分星星
			$str = '';
			for($i=0;$i<5;$i++){
				if($i< $v['avg_point']){
					...
					$str .= "<li class='liposition'></li>";
				}
			}
			$goodses[$k]['xingxing'] = $str;
			/* 查询店铺下商品 */
			$g_info = $GLOBALS['db']->getOne("select min(current_price) from ".DB_PREFIX."deal where supplier_id = ".$v['supplier_id']." limit 1");
			$g_info2 = $GLOBALS['db']->getOne("select min(origin_price) from ".DB_PREFIX."deal where supplier_id = ".$v['supplier_id']." limit 1");
			$g_num = $GLOBALS['db']->getOne("select sum(buy_count) as buy_count from ".DB_PREFIX."deal where supplier_id = ".$v['supplier_id']." group by supplier_id");
			// $g_array= array();
			// foreach ($g_num as $key => $value) {
			// 	// $g_array[] = $value['buy_count'];
			// 	$goodses[$k]['buy_count'] = $value;
				
			// }
			$goodses[$k]['buy_count'] = intval($g_num);
			$goodses[$k]['price'] = round($g_info,2);
			$goodses[$k]['price2'] = round($g_info2,2);
			$goodses[$k]['brief'] = $v['brief'];
		}
