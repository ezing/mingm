	...
	public function index()
	{
		...
		else
		{
			...
			foreach($list as $k=>$v)
			{
			    ...
			    $money_info = $GLOBALS['db']->getOne("select youhui_money from ".DB_PREFIX."youhui where  id= ".$v['youhui_id']);
			    $temp_arr['youhui_money'] =$money_info?$money_info:0;
			    $temp_arr['youhui_type'] = $GLOBALS['db']->getOne("select youhui_type from ".DB_PREFIX."youhui where  id= ".$v['youhui_id']);
				...
				$temp_arr['qrcode'] =  get_abs_img_root(gen_qrcode('youhui'.$v['id']));
