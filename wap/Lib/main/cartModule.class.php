...
// 		}
		...
		$GLOBALS['tmpl']->assign('price',intval($_REQUEST['price']));
...
//		print_r($data);exit;
	...
	public function addcart(){
		...
		$num = $_REQUEST['num'];
		$name_c = $_REQUEST['name_c'];
		$starTime = $_REQUEST['starTime'];
		$endTime = $_REQUEST['endTime'];
		$group_id = $_REQUEST['group_id'];
	    if( !empty($ids)&&(is_array($ids)) ){
			$is_relate = true;
			$data = call_api_core("cart","addcartByRelate",array("ids"=>$ids,"deal_attr"=>$_REQUEST['dealAttrArray']));
		}else{
			$id = intval($ids);
			$pri = intval($price);
			$deal_attr = array();
			...
			$data = call_api_core("cart","addcart",array("id"=>$id,"deal_attr"=>$deal_attr,"price"=>$pri,"group_id"=>$group_id,"name_c"=>$name_c,"starTime"=>$starTime,"endTime"=>$endTime,"num"=>$num));
		}
	    ...
	    if($data['status']==1)
	    {
	    	$ajax_data['jump'] = wap_url("index","cart",array('price'=>$pri));
	    }
