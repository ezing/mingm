...
class uc_collectModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$data = call_api_core("uc_collect","index",$param);
	...
	public function youhui_collect()
	{
		...
		$param=array();
		$param['page'] = intval($_REQUEST['page']);
		$data = call_api_core("uc_collect","youhui_collect",$param);
	...
	public function event_collect()
	{
		...
		$param=array();
		$param['page'] = intval($_REQUEST['page']);
		$data = call_api_core("uc_collect","event_collect",$param);
	...
	public function store_collect(){
		...
		$param=array();
		$param['page'] = intval($_REQUEST['page']);
		$data = call_api_core("uc_collect","store_collect",$param);
		
		if($data['user_login_status']!=LOGIN_STATUS_LOGINED){
			app_redirect(wap_url("index","user#login"));
			
		}
		$data['collect_list']=$data['store_list'];
		foreach($data['collect_list'] as $k=>$v){
			$data['collect_list'][$k]['url']= wap_url("index","event",array("data_id"=>$v['id']));	
			$data['collect_list'][$k]['sub_name']=$v['name'];		
			$data['collect_list'][$k]['buy_count']=$v['submit_count'];	
