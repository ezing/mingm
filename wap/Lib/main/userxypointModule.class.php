...
class userxypointModule extends MainBaseModule
{
	public function index()
	{
		...
		if($xpoint&&$ypoint)
		{
			...
			$current_city = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_city where is_effect = 1 and pid>0 and LOCATE(name,'".$data['district']."')");
			if(empty($current_city))
				$current_city = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_city where is_effect = 1 and pid>0 and LOCATE(name,'".$data['city']."')");
