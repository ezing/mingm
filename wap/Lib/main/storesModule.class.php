...
class storesModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$data = call_api_core("stores","index",$param);
// 		print_r($data);exit;
...
//		
		...
		$GLOBALS['tmpl']->assign("cate_id",intval($_REQUEST['cate_id']));		
		if(intval($_REQUEST['cate_id'])==21){
			// var_dump($data);die;
			$GLOBALS['tmpl']->display("hotel.html");
		}elseif(intval($_REQUEST['cate_id'])==20){
			
			$GLOBALS['tmpl']->display("hairdressing.html");
		}else{
			// var_dump($data['item'][0]['buy_count']);die;
